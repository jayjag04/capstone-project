...
func (s *SmartContract) InitLedger(ctx contractapi.TransactionContextInterface) error {
	...
	for i1, contract := range contracts {
		contract.ContractFor = strings.ToUpper(contract.ContractFor)
		contractAsBytes, _ := json.Marshal(contract)
		err := ctx.GetStub().PutState("CONTRACT" + strconv.Itoa(i1), contractAsBytes)
		if err != nil {
			return fmt.Errorf("Failed to put to world state. %s", err.Error())
		}
	}
...
func (s *SmartContract) CreateAlbumContract(ctx contractapi.TransactionContextInterface, 
	...
	mediaContract := MediaContract{  ContractStatus: contractStatus, ContractFor: strings.ToUpper(albumName), ContractValue: contractValue }
